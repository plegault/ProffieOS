...
// hex 8 bit
// v1 of this hex color systems just a copy of color 8/16 and addeding the colors and see where it gets me


class hex8 {
public:
    hex8() : r(0), g(0), b(0), a(0), w(0), u(0) {}
    hex8(uint8_t r_, uint8_t g_, uint8_t b_, uint8_t a_, uint8_t w_, uint8_t u_) : r(r_), g(g_), b(b_), a(a_), w(w_), u(u_) {}
    // x = 0..256
    hex8 mix(const hex8& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex8(((256 - x) * r + x * other.r) >> 8,
            ((256 - x) * g + x * other.g) >> 8,
            ((256 - x) * b + x * other.b) >> 8,
            ((256 - x) * a + x * other.a) >> 8,
            ((256 - x) * w + x * other.w) >> 8,
    ...
    // x = 0..16384
    hex8 mix2(const hex8& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex8(((16384 - x) * r + x * other.r) >> 14,
            ((16384 - x) * g + x * other.g) >> 14,
            ((16384 - x) * b + x * other.b) >> 14,
    ...
    // x = 0..32768
    hex8 mix3(const hex8& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex8(((32768 - x) * r + x * other.r) >> 15,
            ((32768 - x) * g + x * other.g) >> 15,
            ((32768 - x) * b + x * other.b) >> 15,
    ...
    uint8_t select(const hex8& other) const {
        uint8_t ret = 255;
        if (other.r) ret = std::min<int>(ret, r * 255 / other.r);
        if (other.g) ret = std::min<int>(ret, g * 255 / other.g);
        if (other.b) ret = std::min<int>(ret, b * 255 / other.b);
    ...
    hex8 operator*(uint8_t v) const {
        return hex8(r * v / 255, g * v / 255, b * v / 255, a * v / 255, w * v / 255, u * v / 255);
    }

    hex8 operator|(const hex8& other) const {
        return hex8(std::max(r, other.r), std::max(g, other.g), std::max(b, other.b), std::max(a, other.a), std::max(w, other.w), std::max(u, other.u));
    }
...
class hex16 {
public:
    constexpr hex16() : r(0), g(0), b(0), a(0), w(0), u(0) {}
    hex16(const hex8& c) : r(c.r * 0x101), g(c.g * 0x101), b(c.b * 0x101), a(c.a * 0x101), w(c.w * 0x101), u(c.u * 0x101) {}
    constexpr hex16(uint16_t r_, uint16_t g_, uint16_t b_, uint16_t a_, uint16_t w_, uint16_t u_) : r(r_), g(g_), b(b_), a(a_), w(w_), u(u_) {}
    // x = 0..256
    hex16 mix(const hex16& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex16(((256 - x) * r + x * other.r) >> 8,
            ((256 - x) * g + x * other.g) >> 8,
            ((256 - x) * b + x * other.b) >> 8,
            ((256 - x) * a + x * other.a) >> 8,
            ((256 - x) * w + x * other.w) >> 8,
    ...
    hex16 mix_clamped(const hex16& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex16(clampi32(((256 - x) * r + x * other.r) >> 8, 0, 65536),
            clampi32(((256 - x) * g + x * other.g) >> 8, 0, 65536),
            clampi32(((256 - x) * b + x * other.b) >> 8, 0, 65536),
    ...
    // x = 0..16384
    hex16 mix2(const hex16& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex16(((16384 - x) * r + x * other.r) >> 14,
            ((16384 - x) * g + x * other.g) >> 14,
            ((16384 - x) * b + x * other.b) >> 14,
            ((16384 - x) * a + x * other.a) >> 14,
            ((16384 - x) * w + x * other.w) >> 14,
    ...
    // x = 0..32768
    hex16 mix3(const hex16& other, int x) const {
        // Wonder if there is an instruction for this?
        return hex16(((32768 - x) * r + x * other.r) >> 15,
            ((32768 - x) * g + x * other.g) >> 15,
            ((32768 - x) * b + x * other.b) >> 15,
            ((32768 - x) * a + x * other.a) >> 15,
            ((32768 - x) * w + x * other.w) >> 15,
    ...
    uint16_t select(const hex16& other) const {
        uint32_t ret = 65535;
        uint32_t tmp = 65535;
        if (other.r) ret = std::min<uint32_t>(ret, r * tmp / other.r);
        if (other.g) ret = std::min<uint32_t>(ret, g * tmp / other.g);
    ...
    //hex8? or 16
    hex8 dither(int n) const {
#if (__CORTEX_M - 0 >= 0x04U)  /* only for Cortex-M4 and above */
        if (n < 0) {
            return hex8(__UQSUB16(r, -n) >> 8,
                __UQSUB16(g, -n) >> 8,
                __UQSUB16(b, -n) >> 8,
                __UQSUB16(a, -n) >> 8,
                __UQSUB16(w, -n) >> 8,
...
#else
        return hex8(clampi32((r + n) >> 8, 0, 255),
            clampi32((g + n) >> 8, 0, 255),
            clampi32((b + n) >> 8, 0, 255),
...
public:
    ...
    hex16 rotate(int angle) const {
        ...
        H += angle;
        return hex16(f(5 * 16384 + H, C, MAX),
            f(3 * 16384 + H, C, MAX),
            f(1 * 16384 + H, C, MAX),
            f(2 * 16384 + H, C, MAX),
            f(4 * 16384 + H, C, MAX),
...
struct hex32 {
    hex32(uint32_t r_, uint32_t g_, uint32_t b_, uint32_t a_, uint32_t w_, uint32_t u_) : r(r_), g(g_), b(b_), a(a_), w(w_), u(u_) {}
    hex16 operator>>(int shift) {
        return hex16(r >> shift, g >> shift, b >> shift, a >> shift, w >> shift, u >> shift);
    }
    hex32 operator+(const hex32& other) { return hex32(r + other.r, g + other.g, b + other.b, a + other.a, w + other.w, u + other.u); }
    uint32_t r, g, b, a, w, u;
};

inline hex32 operator*(const hex16& c, uint16_t x) {
    return hex32(c.r * x, c.g * x, c.b * x, c.a * x, c.w * x, c.u * x);
}
inline hex16 operator+(const hex16& a, const hex16& b) {
    return hex16(a.r + b.r, a.g + b.g, a.b + b.b, a.a + b.a, a.w + b.w, a.u + b.u);
}


struct hexSimpleColor {
    hexSimpleColor() {}
    hexSimpleColor(const hex16& c_) : c(c_) {}
    hex16 c;
    void printTo(Print& p) {
        c.printTo(p);
    }
...
inline hexSimpleColor operator<<(const hexSimpleColor& base, const RGBA_um_hex& over) {
    SCOPED_PROFILER();
    if (!over.alpha) return base;
    return hexSimpleColor((base.c * (32768 - over.alpha) + over.c * over.alpha) >> 15);
}
...
// Unmultiplied RGBA (no overdrive), used as a temporary and makes optimization easier.
struct RGBA_um_hex {
  constexpr RGBA_um_hex(hex16 c_, uint16_t a) : c(c_), alpha(a) {}
  constexpr RGBA_um_hex(const hexSimpleColor& c_) : c(c_.c), alpha(32768) {}
  static RGBA_um_nod Transparent() { return RGBA_um_nhex(hex16(), 0); }
  hex16 c;
  uint16_t alpha;
